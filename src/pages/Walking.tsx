...
import walkingTours from "@/data/hiking.json"; // <-- usar alias @
...
const Walking = () => {
  ...
  useEffect(() => {
    try {
      if (Array.isArray(walkingTours)) {
        setTours(walkingTours as WalkingTour[]);
      } else if ((walkingTours as any)?.data) {
        // por si el JSON viene envuelto en { data: [...] }
        setTours((walkingTours as any).data);
      } else {
        console.warn("walking.json no tiene un array en la raíz:", walkingTours);
        setTours([]);
      }
    } catch (err) {
      console.error("Error cargando walkingTours:", err);
      setTours([]);
    }
  }, []);
  ...
  {/* CSS Personalizado para el efecto Tierra */}
  <style>{`
    @keyframes fadeIn {
    ...
    .underline-earth::after {
      ...
      background: linear-gradient(
        90deg,
        rgba(139, 69, 19, 0.1),
        #d46414, /* Café Tierra / SaddleBrown */
        rgba(139, 69, 19, 0.1)
      );
      border-radius: 4px;
  ...
  <div className="container mx-auto px-4">
    ...
    <div className="text-center mb-16 animate-fadeIn">
      <h2 className="relative inline-block text-4xl md:text-6xl font-bold mb-6 text-primary font-marker underline-earth">
        The Best Walking and Hiking Tours
      </h2>

      <p className="text-lg text-muted-foreground max-w-2xl mx-auto font-body mt-6">
        Discover the rainforest step by step with peaceful trails, hanging bridges, waterfalls, and volcano viewpoints. 
        Your gateway to connect with nature at your own pace.
      </p>
    </div>
          {/* ⭐ GRID */}
