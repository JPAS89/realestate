...
import adventureTours from "@/data/adventure.json"; // <-- usar alias @
...
const Adventure = () => {
  ...
  useEffect(() => {
    try {
      if (Array.isArray(adventureTours)) {
        setTours(adventureTours as AdventureTour[]);
      } else if ((adventureTours as any)?.data) {
        // por si el JSON viene envuelto en { data: [...] }
        setTours((adventureTours as any).data);
      } else {
        console.warn("adventure.json no tiene un array en la raíz:", adventureTours);
        setTours([]);
      }
    } catch (err) {
      console.error("Error cargando adventureTours:", err);
      setTours([]);
    }
  }, []);
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <h2 className="relative inline-block text-4xl md:text-6xl font-bold mb-6 text-primary font-marker underline-aquatic">
        The Best Adventure Tours
      </h2>

      <p className="text-lg text-muted-foreground max-w-2xl mx-auto font-body mt-6">
      Feel the adrenaline of La Fortuna through off-road rides, waterfalls, canyons, and jungle thrills—designed for those who seek excitement and unforgettable moments outdoors.
      </p>
    </div>
                  ...
                  >
                    <div className="relative h-64 overflow-hidden">
                      <img
                        ...
                        className="w-full h-full object-cover transition-transform duration-500 hover:scale-90"
                      />
